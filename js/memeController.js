...
// Duplicaton and misconducted use of globals. gImg ? gCurrTxtIdx? gTxtCounter??  gUserMemes???
let gCurrentPos = {
    x: 0,
    y: 0
}
...
function renderPicsToMainGallry(images = getImagesTorender()) {
    //get images to render from service. 
...
function onCreateNewLine() {
    createNewLine('new line', 30, 'center', 'black', 'white', {
        x: 250,
        y: 250
    });
    gCurrTxtIdx++;
...
function drawText(txts) {
    //foreach. for every cell do something (an iteration) compared to for loop. 
